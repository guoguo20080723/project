...
However, some websites do not allow screen scraping due to the protection policies. If I scrape too fast, some websites may identify and ban my behavior. Sometimes web scraping is considered as an unauthorized use or reproduction of others' work since the process involves copying. It is hard to find the line between legal data collection of public contents and copyright violation. Besides, webscraping is slower than accessing data with an API.

The most common application is social media web scraping. There is no database for people's comments in social media, but through web scraping, it is easy to establish a database and analyze the data. For example, with the data, we can see people's attitude towards a policy using sentiment analysis. Another application is machine learning. Because machine learning models need a large amount of raw data to evolve. Web scraping is an efficient way to improve the reliability and accuracy of machine learning models.
